...
-- Replace <ACTUAL_AUTH_USER_UUID> with a real id from the query above, then run one line only.
-- update public.profiles
-- set role = 'admin', full_name = 'مشرف أزرق', phone = '201000000001'
-- where id = '<ACTUAL_AUTH_USER_UUID>';
...
ملفات `_redirects` و`netlify.toml` تحتوي استثناءات لـ `sw.js` و`manifest.webmanifest` والأيقونات قبل catch-all الخاص بتطبيق SPA. الـ service worker الحالي يستخدم cache version ويمسح الكاش القديم أثناء activate بدون إلغاء تسجيله.
...
- صفحة فارغة بعد تحديث كبير: افتح `/sw.js` أو امسح بيانات الموقع؛ الـ service worker الحالي يمسح الكاش القديم باستخدام cache version جديد.
