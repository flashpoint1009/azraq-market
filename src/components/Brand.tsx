  ...
  return (
    <div className="flex items-center gap-3">
      <div className="grid h-12 w-12 place-items-center overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-azraq-100">
        <img src={import.meta.env.VITE_LOGO_URL || '/icon-192.png'} alt={TENANT_CONFIG.brandName} className="h-full w-full object-cover" />
      </div>
      {!compact && (
        <div>
          <p className="font-display text-2xl font-extrabold text-ink">{TENANT_CONFIG.brandName}</p>
          {TENANT_CONFIG.brandSubtitle && <p className="-mt-1 text-xs font-semibold uppercase tracking-[0.3em] text-azraq-500">{TENANT_CONFIG.brandSubtitle}</p>}
        </div>
      )}
    </div>
  );
