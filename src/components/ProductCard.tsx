    ...
    <article className={`relative overflow-hidden rounded-[20px] border border-slate-100 bg-white p-2 shadow-sm ${list ? 'grid grid-cols-[90px_1fr] gap-3' : 'min-h-[224px] max-h-[245px]'}`}>
      ...
      <Link to={`/products/${product.id}`} className={`block overflow-hidden rounded-2xl bg-[#F4FAFF] ${list ? 'h-24' : 'h-24 sm:h-28'}`}>
        {product.image_1_url ? (
          ...
          <div className="grid h-full place-items-center text-xs font-bold text-azraq-700">صورة المنتج</div>
        )}
      </Link>
      ...
      <div className="flex min-h-0 flex-col pt-2">
        ...
        <div className="mt-auto flex items-center justify-between gap-2 pt-2">
          ...
          <button
            ...
            disabled={!canBuy}
            className="grid h-9 w-9 place-items-center rounded-xl bg-azraq-700 text-white shadow-sm disabled:cursor-not-allowed disabled:opacity-50"
            aria-label="زوّد"
          >
            <Plus size={18} />
          </button>
