...
export function Button({ className = '', children, ...props }: ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      className={`inline-flex items-center justify-center gap-2 rounded-2xl bg-azraq-700 px-4 py-2.5 min-h-[44px] text-sm font-bold text-white shadow-soft transition hover:-translate-y-0.5 hover:bg-azraq-800 disabled:cursor-not-allowed disabled:opacity-60 ${className}`}
      {...props}
    >
      {children}
    </button>
...
export function SecondaryButton({ className = '', children, ...props }: ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      className={`inline-flex items-center justify-center gap-2 rounded-2xl border border-azraq-100 bg-white px-4 py-2.5 min-h-[44px] text-sm font-bold text-azraq-800 shadow-sm transition hover:-translate-y-0.5 hover:border-azraq-200 ${className}`}
      {...props}
    >
      {children}
    </button>
...
export function Input(props: InputHTMLAttributes<HTMLInputElement>) {
  return <input {...props} className={`w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 min-h-[44px] text-sm outline-none transition focus:border-azraq-400 focus:ring-2 focus:ring-azraq-100 ${props.className ?? ''}`} />;
}
...
export function Select(props: SelectHTMLAttributes<HTMLSelectElement>) {
  return <select {...props} className={`w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 min-h-[44px] text-sm outline-none transition focus:border-azraq-400 focus:ring-2 focus:ring-azraq-100 ${props.className ?? ''}`} />;
}
  ...
  return (
    <div className="mb-3 flex flex-row items-start justify-between gap-2">
      <div className="min-w-0">
        <p className="text-2xs font-bold uppercase tracking-[0.2em] text-azraq-500">أزرق ماركت</p>
