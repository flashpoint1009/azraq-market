...
export function AdminPurchasesPage() {
  ...
  const save = async (event: FormEvent) => {
    ...
    const invoice = await supabase
      ...
      .insert({ supplier_name: supplierName || null, total_amount: total, created_by: profile?.id || null })
      .select()
      .single();

    if (invoice.error || !invoice.data) {
      console.error('PURCHASE_INVOICE_SAVE_FAILED', invoice.error);
      setSaving(false);
      toast.error('تعذر تحميل البيانات، حاول مرة أخرى');
      return;
    }

    const invoiceItems = cleanItems.map((item) => ({
    ...
    const result = await supabase.from('purchase_invoice_items').insert(invoiceItems);
    if (result.error) {
      console.error('PURCHASE_INVOICE_ITEMS_SAVE_FAILED', result.error);
      setSaving(false);
      toast.error('تعذر تحميل البيانات، حاول مرة أخرى');
      return;
    }
