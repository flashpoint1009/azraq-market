...
export function CustomerHome() {
  ...
  const { data, loading, error } = useSupabaseQuery(async () => {
    ...
    if (categoriesResult.error) {
      console.error('CATEGORIES_FETCH_ERROR', categoriesResult.error);
  ...
  const dealProducts = useMemo(() => products.filter((product) => (product.discount_type ?? 'none') !== 'none' && Number(product.discount_value || 0) > 0).slice(0, 8), [products]);
          ...
          <aside className="absolute inset-y-0 right-0 w-[82vw] max-w-sm rounded-l-[28px] bg-white p-4 shadow-2xl">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <h2 className="font-display text-xl font-extrabold text-ink">{profile?.full_name || profile?.phone || 'عميل'}</h2>
