    ...
    <div className="pb-24">
      ...
      <div className="mb-3 rounded-2xl bg-azraq-900 p-3 text-white shadow-soft">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[11px] font-bold text-azraq-300">عميل</p>
            <h2 className="truncate font-display text-base font-extrabold">{profile?.full_name || 'أزرق ماركت'}</h2>
            {profile?.phone && <p className="text-[11px] font-bold text-white/60" dir="ltr">{profile.phone}</p>}
          </div>
        </div>
        {profile?.address && (
          <div className="mt-2 flex items-start gap-1.5 rounded-xl bg-white/10 px-2.5 py-1.5">
            ...
            <p className="text-[11px] font-bold text-white/70 line-clamp-1">{profile.address}</p>
          </div>
        )}
      </div>
      ...
      <Card className="p-3">
        <form onSubmit={submit} className="grid gap-3">
          <div>
            <label className="mb-1 block text-[11px] font-bold text-slate-500">الاسم بالكامل</label>
            <Input value={fullName} onChange={(event) => setFullName(event.target.value)} placeholder="الاسم بالكامل" />
          </div>
          <div>
            <label className="mb-1 block text-[11px] font-bold text-slate-500">رقم الموبايل</label>
            <Input value={profile?.phone || ''} disabled placeholder="رقم الموبايل" />
          </div>
          <div>
            <label className="mb-1 block text-[11px] font-bold text-slate-500">العنوان</label>
            <Textarea
              value={address}
          ...
          <div className="rounded-xl border border-slate-100 bg-slate-50 p-3">
            ...
            {locationCoords ? (
              <div className="mb-2 flex items-center gap-2 rounded-xl bg-emerald-50 px-3 py-2 text-[11px] font-bold text-emerald-700">
                <CheckCircle2 size={14} />
                <span>تم تحديد الموقع ({locationCoords.lat.toFixed(4)}, {locationCoords.lng.toFixed(4)})</span>
              </div>
            ) : (
              <p className="mb-2 text-[11px] font-bold text-slate-400">لم يتم تحديد الموقع بعد</p>
            )}
            <button
              type="button"
            ...
            <p className="mt-1.5 text-center text-[10px] font-bold text-slate-400">يساعد المندوب في الوصول إليك بدقة</p>
          </div>
